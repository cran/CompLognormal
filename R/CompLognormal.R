...
pcomplnorm<-function(x, spec, sigma=1, theta=1, log.p=FALSE, ...)
{
        f2=function (z, ...) {do.call(paste("d",spec,sep=""),list(z, ...))}
        F2=function (z, ...) {do.call(paste("p",spec,sep=""),list(z, ...))}
        ff=function (z) {f2(z, ...)}
	...
	cdf=x
        cdf[x<=theta&log.p==FALSE]=F1(x[x<=theta]) / ((1+phi)*F1(theta))
        cdf[x<=theta&log.p==TRUE]=F1L(x[x<=theta])-log(1+phi)-F1L(theta)
        cdf[x>theta&log.p==FALSE]=(1/(1+phi)) + (phi/(1+phi))*(F2(x[x>theta], ...)-F2(theta, ...))/(1-F2(theta, ...))
        cdf[x>theta&log.p==TRUE]=-log(1+phi)+log(1 + phi*(F2(x[x>theta], ...)-F2(theta, ...))/(1-F2(theta, ...)))
	return(cdf)
}



qcomplnorm<-function(p, spec, sigma=1, theta=1, log.p=FALSE, ...)
{
